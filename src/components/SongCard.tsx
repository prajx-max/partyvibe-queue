  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg transition-all disabled:opacity-50 active:scale-90 min-h-[44px] min-w-[44px] justify-center ${
          song.user_voted
            ? 'bg-primary/20 text-primary neon-border'
            : 'bg-muted text-muted-foreground active:text-primary active:bg-primary/10'
        }`}
      >
      ...
      {isHost && (
        <div className="flex items-center gap-1 shrink-0">
          {onPlay && !isCurrentSong && (
            <Button variant="ghost" size="icon" className="h-10 w-10 text-muted-foreground hover:text-primary" onClick={() => onPlay(song.id)} title="Play this song now">
              <Play className="h-4 w-4" />
            </Button>
          )}
          {onRemove && (
            <Button variant="ghost" size="icon" className="h-10 w-10 text-muted-foreground hover:text-destructive" onClick={() => onRemove(song.id)} title="Remove song">
